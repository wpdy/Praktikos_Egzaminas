  ...
  if (category === "") {
    return (
      <>
        {showEvent && (
          <div className="goal">
            <img className="image" src={event.image} alt="No Image"></img>
            <h2 className="title">{event.title}</h2>
            <div className="types">
              <span className="project-type">• {event.category}</span>
            </div>
            <p className="description">{event.description}</p>
            <h2 className="date">{event.date.slice(0, 10)}</h2>
            ...
            <div className="fav_container">
              {showFavouriteButton && (
                <button
                  ...
                  className={`${
                    Favorited ? "favouriteIconRed fav" : "favouriteIcon fav"
                  }`}
                >
                  <RxHeartFilled />
                </button>
        ...
        {updateevent && (
          <UpdateEvent event={event} updateBackButton={updateBackButton} />
        )}
      </>
    );
  }
